...
const QUESTIONS = [
  { id: 'goal', question: "Ton objectif principal ?", options: ["Perte de poids", "Prise de masse", "Remise en forme", "Performance"] },
  { id: 'level', question: "Ton niveau actuel ?", options: ["Débutant", "Intermédiaire", "Avancé"] },
  { id: 'days', question: "Fréquence d'entraînement ?", options: ["2 jours", "3 jours", "4 jours", "5 jours+"] },
...
export default function Onboarding({ onComplete }: { onComplete: (data: Record<string, string>) => void }) {
  ...
  const handleAnswer = (option: string) => {
    if (isTransitioning) return; // Empêche le double clic
      ...
      <div className="max-w-xl w-full bg-[#0d0d0d] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
          <div className="flex justify-between mb-8">
            <button onClick={() => setCurrentStep(prev => prev - 1)} disabled={currentStep === 0 || isTransitioning} className="disabled:opacity-0 text-slate-400 hover:text-white"><ArrowLeft /></button>
          ...
          <div className="space-y-3">
            {QUESTIONS[currentStep].options.map((opt) => (
              <button key={opt} onClick={() => handleAnswer(opt)} disabled={isTransitioning} className={`w-full p-4 rounded-xl border text-left transition-all flex justify-between items-center ${answers[QUESTIONS[currentStep].id] === opt ? 'border-blue-500 bg-blue-500/10 text-white' : 'border-slate-700 bg-[#1a1a1a] text-slate-400 hover:border-slate-500'}`}>
