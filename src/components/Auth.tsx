  ...
  if (isEmailSent) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-[#0d0d0d] p-8 rounded-2xl border border-slate-800 text-center space-y-6 shadow-2xl">
          ...
          <p className="text-slate-400">Un lien de confirmation a été envoyé. Cliquez ci-dessous pour continuer le test.</p>
          <button onClick={onConfirm} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition-all">
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4">
      <div className="max-w-md w-full bg-[#0d0d0d] p-8 rounded-2xl border border-slate-800 shadow-2xl space-y-8">
        <div className="text-center">
          ...
          <p className="text-slate-400 mt-2">Créez votre compte coach</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all">
            {isLoading ? <Loader2 className="animate-spin" /> : "S'inscrire maintenant"}
          </button>
        </form>
      </div>
    </div>
